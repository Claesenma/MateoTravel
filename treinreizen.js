...
function get_stations() {
  ...
  fetch('https://tequila-api.kiwi.com/locations/dump?locale=nl-NL&location_types=station&sort=name&limit=4000', {headers: headers2})
  .then(response => {
  if (!response.ok) {throw new Error(`Request failed with status ${response.status}`)}
    return response.json()
    document.write("error")
...
function add_input_options(plaats="vertrekplaats") {
  ...
  var element_count = 0;
  for (var element in window.stations_data_dict) {
    var value = input.value.toLowerCase();
    var element_lower = element.toLowerCase();
    var airport = window.stations_data_dict[element];
  ...
  for (var element in window.stations_data_dict) {
    ...
    if (element_lower.includes(value) || airport["city"].toLowerCase().includes(value) || airport["country"].toLowerCase().includes(value) || airport["name"].toLowerCase().includes(value)) {
      ...
      var p1 = document.createElement("p");
      p1.innerHTML = element + ", " + window.stations_data_dict[element]["city"];
      a.appendChild(p1);
      var p2 = document.createElement("p");
      p2.innerHTML = window.stations_data_dict[element]["country"];
      a.appendChild(p2);
      var p3 = document.createElement("p");
      p3.innerHTML = window.stations_data_dict[element]["name"];
      a.appendChild(p3);
...
function zoeken() {
  ...
  if (!(bestemming in window.stations_data_dict)) {
    if (bestemming!=="OVERAL") {
      error = true;
      error_message =  "Deze bestemming werd niet gevonden."
    }
  }
  if (!(vertrekplaats in window.stations_data_dict)) {
    error = true;
...
function show_results(data, ve, be, da, vo, ki, ba, ty, so, munteenheid, st) {
  ...
  for (result in data["data"]) {
    ...
    vertrekplaats_naam_text.innerHTML = window.stations_data_dict[data["data"][result]["flyFrom"]]["name"];
    ...
    plane_img.src = "images/trein_route.png";
    ...
    bestemming_naam_text.innerHTML = window.stations_data_dict[data["data"][result]["flyTo"]]["name"];
...
function show_details(data, id, munteenheid) {
  ...
  if (error == false) {
    ...
    for (n = 0; n < (stops_count+1); n++) {
      ...
      if (result2["flyFrom"] in window.stations_data_dict) {
        var flight_text1 = result2["flyFrom"] + ", " + result2["cityFrom"] + ", " + window.stations_data_dict[result2["flyFrom"]]["country"];
      }
      else {
      ...
      route_text_.innerHTML = seconds_to_hours(duur) + " in de trein ("+ airline +")";
      ...
      if (result2["flyTo"] in window.stations_data_dict) {
        var flight_text2 = result2["flyTo"] + ", " + result2["cityTo"] + ", " + window.stations_data_dict[result2["flyTo"]]["country"];
      }
      else {
...
async function show1() {
  ...
  while (loading==true) {
    await delay(1);
    if (window.stations_data_dict!=undefined) {
